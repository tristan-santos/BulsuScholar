...
const MagicBento = ({
  ...
  const bentoItems = Array.isArray(items) && items.length > 0 ? items : DEFAULT_CARD_DATA;
  ...
  return (
    <div className={`magic-bento-root ${className}`.trim()}>
      {enableSpotlight && (
        <GlobalSpotlight
          gridRef={gridRef}
      ...
      <BentoCardGrid gridRef={gridRef}>
        {bentoItems.map((card, index) => {
          const baseClassName =
            `magic-bento-card ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''} ${card.className || ''}`.trim();
          const cardProps = {
