...
1.  **Confirmation Email**
    *   **Trigger:** Student creates an unconfirmed account.
    *   **Content:** Supabase Auth confirmation link delivered through the configured SMTP provider.
2.  **Forgot Password**
...
A backend-only email adapter should handle approved transactional templates. Authentication email is sent by Supabase Auth.

```text`r`nAuthentication messages are generated by Supabase Auth. Other transactional messages are sent only by backend-controlled templates.`r`n```
